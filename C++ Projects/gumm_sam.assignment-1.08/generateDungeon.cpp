    ...
    public:
        ...
        */
        ...
        void process_pc_command(int command) {
            ...
            // Monster collision: kill the monster pc runs into
            for (Monster &m : monsters) {
                if (m.alive &&
                    m.position.x == new_x &&
                    m.position.y == new_y) {
                    m.alive = false;
                    if(m.is_unique) used_uniques.insert(m.NAME);
                    mvprintw(0, 0, "You killed monster %c!", m.SYMB);
                    break;
                }
            }
        ...
        void render_dungeon_ncurses() {
            ...
            for (int y = 0; y < H; ++y) {
                for (int x = 0; x < W; ++x) {
                    ...
                    /* 1) base terrain or fog */
                    char ch = terrain_at(x,y);
        
                    /* 2) object overlay */
                    if (const Object* o = visible_object_at(x,y)) {
                        attron(COLOR_PAIR(o->color));
                        ch = o->glyph();
                        attroff(COLOR_PAIR(o->color));
                    }
        
                    /* 3) monster overlay (lambda handles fog rule) */
                    if (char m = visible_monster_at(x,y))
                        ch = m;
